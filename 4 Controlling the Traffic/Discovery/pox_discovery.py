...
class NetworkTopology:
    ...
    def createDOTfile(self):
        ...
        for s in self.listSwitches:
            ...
            content += "\tsubgraph cluster_s" + \
                str(switch.dpid) + " {\n\t\tlabel=\"" + \
                dpidToStr(switch.dpid) + "\";\n"
            # Add ports of switches
            for p in switch.ports:
                content += "\t\tport" + str(switch.dpid) + "_" + str(p) + "[label=\"port: " + str(
                    p) + "\\n" + str(switch.ports[p].hwAddr) + "\", shape=box];\n"
            content += "\t}\n"
        # Add links between switches
        for link in self.linkList:
            content += "\tport" + str(link.dpid1) + "_" + str(
                link.port1) + " -- port" + str(link.dpid2) + "_" + str(link.port2) + "\n"
        content += "}"
    ...
    def printTopology(self):
        ...
        for s in self.listSwitches:
            ...
            for p in switch.ports:
                print "\t\tName", switch.ports[p].name, "No", \
                switch.ports[p].number, "MAC", switch.ports[p].hwAddr, \
                "Entites", switch.ports[p].entities
        # Print all links
        print "Links"
        for link in self.linkList:
            print "\t", link
    ...
    def removeLink(self, linkRemove):
        ...
        for link in self.linkList:
            if ((link.dpid1 == linkRemove.dpid1 and link.dpid2 == linkRemove.dpid2 and link.port1 == linkRemove.port1 and link.port2 == linkRemove.port2) or (
    ...
    def addLink(self, linkAdd):
        ...
        for link in self.linkList:
            if ((link.dpid1 == linkAdd.dpid1 and link.dpid2 == linkAdd.dpid2 and link.port1 == linkAdd.port1 and link.port2 == linkAdd.port2) or (
    ...
    def removeSwitch(self, switchRemove):
        ...
        if switchRemove in self.listSwitches:
...
def launch():
    ...
    def addEventListeners():
        ...
        core.topology.addListenerByName(
            "SwitchLeave",
            topo._handle_removeSwitch)
        core.openflow.addListenerByName(
            "ConnectionDown",
            topo._handle_connectionDown)
